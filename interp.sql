...
output_dataframe = pd.DataFrame(rsxVl)
output_dataframe[2] = pd.DataFrame(rsyVl)
output_dataframe[3] = pd.DataFrame(numpy.array(xx(rsxVl["X"], rsyVl["Y"])[0]))
for i in range(len(rsxVl["X"])):
	print("x : {0}, y : {1}, z : {2}".format(rsxVl["X"][i], rsyVl["Y"][i], xx(rsxVl["X"][i], rsyVl["Y"][i])[0]))

'
,@input_data_1 = N'SELECT * FROM #XY'
,@input_data_1_name = N'XY'
,@output_data_1_name = N'output_dataframe'
,@params = N'@xValues nvarchar(max), @yValues nvarchar(max), @RSxValues nvarchar(max), @RSyValues nvarchar(max)'
,@xValues = @xVa
,@yValues = @yVa
,@RSxValues = @RSVxa
,@RSyValues = @RSVya
with result sets ((X int, Y int, XY float))